...
var uniqueRecordSet = _.uniqBy(rows, 'invoiceNo');
	...
	for(var i = 0; i < uniqueRecordSet.length; i++){
	  ...
	  for(var j=0;j<resultSet.count;j++){
	   ...
	   DateTime: resultSet.data[j].dateTime.getDate()+"-"+(resultSet.data[j].dateTime.getMonth() + 1)+"-"+resultSet.data[j].dateTime.getFullYear(),
...
function tablenew(data, columns) {
	...
	return {
			...
			body: [
			 ...
			 'Partner id:000'+RestaurantId+'\nBill number :'+InvoiceNumber+'\nBill date :'+end.getDate()+"-"+(end.getMonth() + 1)
			 +"-"+end.getFullYear()+'\nBill period :'+start.getDate()+"-"+(start.getMonth() + 1)+"-"+start.getFullYear()+' to '
			 +end.getDate()+"-"+(end.getMonth() + 1)+"-"+end.getFullYear()+'\nPay By Date :'+dueDate+
...
pdfDoc.pipe(fs.createWriteStream('invoice/000'+RestaurantId+'-'+end.getDate()+"-"+(end.getMonth() + 1)+"-"+end.getFullYear()+'.pdf')).on('finish',function(){
});
pdfDoc.end();
