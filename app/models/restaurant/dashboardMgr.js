...
function doRatingCalculation(values){
    ...
    ratingSummary.averageRating = ratingCount != 0 ? ((totalRating/3)/ratingCount).toFixed(1) : 0;
...
function getRestTxnSummary(req, response){
    ...
    .then(rid => {
        ...
        .then(values => {
            var currentIntervalInfo = doSummaryCalculation(values[0] == null ? 0 : values[0],values[5] == null ? 0 : values[5]);
            var previousIntervalInfo = doSummaryCalculation(values[1] == null ? 0 : values[1],values[6] == null ? 0 : values[6]);
