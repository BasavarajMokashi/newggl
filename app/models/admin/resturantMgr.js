...
function starllyResturants(req, response){
    dbutils.getStarllyResturants(req.params.status)
    .then(values =>{
...
function setRestaurantAsLive(req, response){
    dbutils.setRestaurantAsLive(req.params.restid)
    .then(values =>{
        response.status(200).json(values);
    },error =>{
        response.status(500).json({"status": error });
...
module.exports = {
    getStarllyResturants : function(req, response) {
        starllyResturants(req,response);
    },
    setRestaurantAsLive : function(req, response){
        setRestaurantAsLive(req, response);
    }
